...
//
func NtpGetTime(host string) (time.Time, error) {
	if host == "" {
		return time.Now(), nil
	}
	...
	if err != nil {
		return time.Now(), fmt.Errorf("failed to connect:", err)
	}
	defer conn.Close()
	if err := conn.SetDeadline(time.Now().Add(15 * time.Second)); err != nil {
		fmt.Errorf("failed to set deadline: ", err)
	}
	...
	if err := binary.Write(conn, binary.BigEndian, req); err != nil {
		return time.Now(), fmt.Errorf("failed to send request: %v", err)
	}

	// block to receive server response
	rsp := &packet{}
	if err := binary.Read(conn, binary.BigEndian, rsp); err != nil {
		return time.Now(), fmt.Errorf("failed to read server response: %v", err)
	}
